...
var allowedPolicyTrustTiers = []string{"tier0", "tier1", "tier2", "tier3"}
var allowedPolicySensitivity = []string{"A", "B", "C", "D"}
...
func (p ProviderPolicy) Validate() error {
	...
	for idx, provider := range p.Providers {
		...
		for _, tier := range provider.TrustTiers {
			if !slices.Contains(allowedPolicyTrustTiers, tier) {
		...
		for _, class := range provider.Sensitivity {
			if !slices.Contains(allowedPolicySensitivity, class) {
